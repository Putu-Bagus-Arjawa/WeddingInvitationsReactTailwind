    ...
    <section className="md:min-h-screen h-auto" id="couple" ref={refer}>
      ...
      <section className="p-8 md:min-h-screen h-auto flex md:flex-row flex-col justify-center gap-x-20 items-center gap-y-8">
        {couple.map((items, idx)=>(
          <motion.div
            key={idx}
            initial ={idx%2 == 0?{ opacity: 0, x:-500 }:{opacity:0, x: 500}}
            animate ={isInView?{ opacity: 1, x :0 }: {}}
            transition={{type:"spring", duration: 1.2, bounce:0.4, ease:"easeInOut"}}
          >
              <Card>
                <div className="flex flex-col items-center max-w-50 md:max-w-80">
                    <img src={items.image} alt="" className="h-50 w-50 md:mb-8 mb-4 rounded-full" />
                    <h1 className="font-playfair md:text-2xl text-xl text-rose-500">{items.name}</h1>
                    <p className="text-neutral-500 font-quickSand text-center md:text-xl text-md">{items.kid}</p>
                </div>
            </Card>
          </motion.div>
        ))}
